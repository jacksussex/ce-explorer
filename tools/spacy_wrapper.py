import spacy, fields, pandas as pd
...
endpoint = "https://nerd.huma-num.fr/nerd/service"

## Extract, combine and rank entities by frequency in the text
## Extract, combine and rank entities by frequency in the text

def spacy_doc_col(column):
    return column + '_' + fields._spacy_doc
...
class SpaCyWrapper():
    ...
    def __init__(self,model='en_core_web_sm', exclusions=None, custom_stopwords={}, include_fishing=False):
        ...
        if include_fishing:
            self.nlp.add_pipe('entityfishing', config={"extra_info": True, "api_ef_base": endpoint})
        if custom_stopwords:
            self.nlp.Defaults.stop_words |= set(custom_stopwords)
        self.progress = 0

    def get_stopwords(self):
        return self.nlp.Defaults

    def nlp_dataframe(self,dataframe, text_column, progress, max_length=1000000):
        """Parses all texts using the nlp pipeline specific on construction"""
        self.progress = 0
        # ddf = dd.from_pandas(dataframe, npartitions=10)
        dataframe[spacy_doc_col(text_column)] = dataframe[text_column].apply(lambda x: self.nlp_doc(x,len(dataframe), progress))
        # ddf[fields._spacy_doc] = ddf[text_column].apply(lambda x: self.nlp_doc(x, update_progress))
    ...
    def nlp_doc(self, text, dataset_size, progress):
        self.progress += 1
        progress_val = (self.progress / dataset_size) * 100
    ...
    def get_entities(self, spacy_doc, labels):
        ...
        for ent in spacy_doc.ents:
            if ent.label_ in ner_texts:
                ner_texts[ent.label_].append(ent.dataset_name)
        return ner_texts

    def entity_label_df(self, dataframe, labels, column):
        dataframe = dataframe.merge(dataframe[column].apply(lambda doc: pd.Series(self.get_entities(doc, labels=labels))),
